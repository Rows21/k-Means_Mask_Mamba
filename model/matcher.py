...
def compute_mask_similarity(inputs: torch.Tensor, targets: torch.Tensor,
    ...
    inputs = F.softmax(inputs, dim=0) # N x HWD
...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def memory_efficient_forward(self, outputs, targets, cls_list):
        ...
        for b in range(bs):
            with autocast(enabled=False):
                cls_label = [organ - 1 for organ in cls_list[b]]
                class_similarity = compute_class_similarity(outputs["pred_logits"][b].float(), cls_label)
            out_mask = outputs["pred_masks"][b].flatten(1)  # [num_queries, H_pred, W_pred]
            # gt masks are already padded when preparing target
            tgt_mask = targets[b].to(out_mask).flatten(1)
            with autocast(enabled=False):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, cls_list):
        ...
        """
        return self.memory_efficient_forward(outputs, targets, cls_list)
