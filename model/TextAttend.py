...
class TextAttend(nn.Module):
    ...
    def __init__(self,
                 *,
                 ...
                 assign_eps=1.,
                 gumbel_tau=1.):
        super(TextAttend, self).__init__()
        self.dim = dim
        self.hard = hard
        self.gumbel = gumbel
        self.sum_assign = sum_assign
        self.norm_tokens = norm_layer(512)
        ...
        # norm on x
        self.norm_x = norm_layer(512)
        self._pixel_v_conv_bn = ConvBN(dim, 512, kernel_size=1, bias=False,
                                          norm='3d', act=None)
        trunc_normal_(self._pixel_v_conv_bn.conv.weight, std=512)
        self.pre_assign_attn = CrossAttnBlock(
            dim=512, num_heads=num_heads, mlp_ratio=4, qkv_bias=True, norm_layer=norm_layer, post_norm=True)
        ...
        self.norm_new_x = norm_layer(512)
        self.mlp_channels = Mlp(512, mlp_ratio*512, out_dim)
        if out_dim is not None and dim != out_dim:
            self.reduction = nn.Sequential(norm_layer(512), nn.Linear(512, out_dim, bias=False))
        else:
            self.reduction = nn.Identity()

    def project_token(self, tokens):

        projected_tokens = self.project_layer(tokens)
        return projected_tokens
    
    
    def forward(self, x, task_embs, anomaly_mask, return_attn=False):
        """
        Args:
        ...
        Returns:
            new_x (torch.Tensor): [B, S_2, C], S_2 is the new number of
                group tokens
        """
        #task_embs = self.project_token(task_embs)
        task_embs = self.norm_tokens(task_embs)
        x = self._pixel_v_conv_bn(x)
        x = self.norm_x(x.flatten(start_dim=2, end_dim=4).transpose(1, 2))
        anomaly_mask = anomaly_mask.flatten(start_dim=2, end_dim=4)
        # [B, S_2, C]
        task_embs = self.pre_assign_attn(task_embs, x, mask=anomaly_mask)
        
        # diffusion attention block
        task_out, attn_dict = self.assign(task_embs, x, return_attn=return_attn)
        task_out += task_embs
...
class TextAttend_H(nn.Module):
    ...
    def __init__(self,
                 *,
                 ...
                 assign_eps=1.,
                 gumbel_tau=1.):
        super(TextAttend_H, self).__init__()
        self.dim = dim
        self.hard = hard
        self.gumbel = gumbel
        self.sum_assign = sum_assign
    ...
    def forward(self, x, task_embs, return_attn=False):
        """
        Args:
            x (torch.Tensor): image tokens, [B, L, C]
            group_tokens (torch.Tensor): group tokens, [B, S_1, C]
        ...
        x = self.norm_x(x.flatten(start_dim=2, end_dim=4).transpose(1, 2))
        # [B, S_2, C]
        task_out = self.pre_assign_attn(task_embs, x)
        task_out += task_embs
        task_out = self.reduction(task_out) + self.mlp_channels(self.norm_new_x(task_out))
...
class TextAttend_L(nn.Module):
    ...
    def __init__(self,
                 *,
                 ...
                 assign_eps=1.,
                 gumbel_tau=1.):
        super(TextAttend_L, self).__init__()
        self.dim = dim
        self.hard = hard
        self.gumbel = gumbel
        self.sum_assign = sum_assign
