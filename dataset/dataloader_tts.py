...
class_map_part_organs_new = {
    ...
    7: "stomach",
    8: "aorta",
    0: "inferior_vena_cava",
    10: "portal_vein_and_splenic_vein",
    11: "pancreas",
    12: "adrenal_gland_right",
    13: "adrenal_gland_left",
    16: "lung_upper_lobe_left",
    16: "lung_lower_lobe_left",
    17: "lung_upper_lobe_right",
...
totalseg_taskmap_set = {
    ...
    'organs': class_map_part_organs_new,
...
class LoadImageh5d_totoalseg(MapTransform):
    def __init__(
        self,
        keys: KeysCollection,
        map_type,
...
def get_loader_tts(args):
    ...
    val_transforms = Compose(
        [
            LoadImaged_totoalseg(keys=["image"], map_type='organs'),
            AddChanneld(keys=["image", "label"]),
            Orientationd(keys=["image", "label"], axcodes="RAS"),
            Spacingd(
                keys=["image", "label"],
    ...
    for item in args.dataset_list:
        train_txt_path = os.path.join(args.data_txt_path, 'train_' + str('1081') + '.txt')
        for line in open(train_txt_path):
            name = line.strip().split('\t')[0]
            train_img_path = os.path.join(args.data_root_path, name, 'ct.nii.gz')
            train_lbl_path = os.path.join(args.data_root_path, name, 'segmentations/')
    ...
    for item in args.dataset_list:
        ...
        for line in open(val_txt_path):
            name = line.strip().split('\t')[0]
            val_img_path = os.path.join(args.data_root_path, name, 'ct.nii.gz')
            val_lbl_path = os.path.join(args.data_root_path, name, 'segmentations/')
    ...
    for item in args.dataset_list:
        ...
        for line in open(test_txt_path):
            name = line.strip().split('\t')[0]
            test_img_path = os.path.join(args.data_root_path, name, 'ct.nii.gz')
            test_lbl_path = os.path.join(args.data_root_path, name, 'segmentations/')
